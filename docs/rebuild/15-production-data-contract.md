...
- `형합수 추정`: 사출 모니터링 형합수와 계획 cavity를 이용해 산출한 품번별 추정 생산량이다. 금형/코어 교체 신호가 확인되면 해당 전환 시점의 누적 생산량을 기준으로 배분하고, 전환 신호가 없지만 Part No.가 같거나 마지막 2자리 숫자만 다른 동일 cavity의 연속 코어 작업지시로 판단되면 앞 작업지시 계획량까지 채운 뒤 나머지를 다음 작업지시로 넘긴다.
...
- 전환 신호가 없고 연속 작업지시의 Part No가 완전히 같으면, 작업지시 번호/Lot가 달라도 같은 품목을 나누어 지시한 것으로 보고 작업지시 순서대로 앞 작업지시 계획량을 먼저 채운 뒤 잔량을 다음 작업지시에 배정한다. 이 경우 모델명이나 제품군 텍스트가 일부 누락 또는 표기 차이를 보여도 Part No와 cavity가 같으면 자동 이월할 수 있다.
- 전환 신호가 없고 연속 작업지시가 동일 길이의 충분히 긴 Part No.에서 마지막 2자리 숫자만 다른 코어 파생 품번이면, 모델명/제품군 표기 차이가 있어도 동일 cavity 기준으로 앞 작업지시를 계획량까지 채운 뒤 남은 형합수를 다음 작업지시로 자동 이월한다.
...
- 같은 방식으로 2호기에서 `ABJ76507604`와 `ABJ76507601`처럼 동일 길이 Part No의 마지막 2자리만 다른 경우도 코어 파생 작업으로 보고, 명확한 전환 신호가 없어도 앞 작업지시 계획량을 먼저 채운 뒤 뒤 작업지시로 이어 배정한다.
- 같은 설비에서 `ACQ30776309`가 작업지시 번호 또는 Lot만 달라 `1,000개`, `920개` 두 지시로 나뉘고, MES 모니터링 형합수 추정이 `1,974개`라면 생산대시보드는 첫 번째 작업지시에 `1,000개`, 두 번째 작업지시에 `974개`를 배정한다. MES 모니터링의 입고/형합수 비교 카드는 Part No 기준으로 합산해 `1,974 / 1,920`을 보여주되, 생산대시보드는 작업지시별 진행 공유를 위해 분리 표시한다.
