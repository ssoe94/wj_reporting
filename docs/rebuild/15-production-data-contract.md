...
- 1차 매칭: `설비 + 작업지시 Part No.`가 직접 일치하면 해당 작업지시에 ZS 입고를 배정한다.
...
- `형합수 추정`: 사출 모니터링 형합수와 계획 cavity를 이용해 산출한 품번별 추정 생산량이다. 금형/코어 교체 신호가 확인될 때만 다음 작업지시로 생산량을 넘긴다.
- `ZS 입고`: MES `ZS` 생산보고의 `report_qty` 합계다.
- `차이`: `ZS 입고 - 형합수 추정`으로 계산한다.
- 화면의 대표 Part No.는 작업지시 Part No.를 우선 표시하고, ZS에서 들어온 반제품 Part No.는 보조 텍스트로 남긴다.
- 화면에는 `직접 Part No.`, `모델/품번 후보`, `미매칭` 중 어떤 기준으로 붙었는지 표시한다.
- 백엔드 stats 응답은 같은 MES 그룹 안의 `material_name` 후보를 `mes_material_names` 배열로 보존해, 프런트가 첫 번째 material 이름만 보고 매칭을 놓치지 않게 한다.

상태:

- `수량 일치`: 형합수 추정 생산량과 ZS 입고량이 같다.
- `ZS 입고 부족`: 형합수 추정 생산량이 있는데 ZS 입고가 적다.
- `ZS 입고 초과`: ZS 입고가 형합수 추정 생산량보다 많다.
- `ZS 미입고`: 형합수 추정 생산량은 있으나 ZS 입고가 없다.
- `형합수 없음`: ZS 입고는 있으나 해당 설비·품번의 형합수 추정 실적이 없다.

이 비교는 생산실적을 대체하지 않는다. 목적은 사출과/생산 담당자가 “이미 형합수로 생산이 보이는데 ZS 입고가 늦거나 누락된 건”과 “입고는 있는데 모니터링상 생산 근거가 약한 건”을 빠르게 찾는 것이다.
