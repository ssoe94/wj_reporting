...
## 14. 가공 MES 우선 수기 보정 프로세스

가공 라인의 생산 집계는 별도 기준이 필요하다. 기본 실적은 MES `报工`를 우선 사용하고, 수기 입력은 MES에 아직 잡히지 않은 실제 생산을 보정하는 용도로만 사용한다.
...
1. 시스템이 MES 가공 `报工` 데이터를 자동 수집한다.
2. 생산 담당자는 하루 1~2회 생산대시보드의 가공 provision 영역을 확인한다.
3. 화면은 업무일 기준 `당일 + 다음날 + 다다음날` 계획을 이어서 보여준다.
4. 담당자는 이미 생산했지만 MES에 없는 row만 선택해 수기 보정한다.
5. 수기 보정에는 생산수량, 불량 유형/수량, 보정 사유, 비고를 입력한다.
6. 이후 MES 보고가 들어오면 시스템이 수기 보정과 대사한다.
7. 집계는 MES와 수기를 단순 합산하지 않고, MES가 대체하지 않은 수기 잔량만 반영한다.
...
- MES가 있으면 MES가 우선이다.
- 수기 보정은 누락 보정 레이어다.
- 수기 보정은 실제 생산 업무일과 원래 계획일을 모두 저장한다.
- 선진행 생산은 `business_date < plan_date`로 표현한다.
- 나중에 MES 보고가 들어오면 수기 보정은 `matched`, `partial`, `mismatch` 중 하나로 전이한다.
- `/production`, `/analysis`, AI 브리핑은 모두 대사 후 실적을 사용한다.

## 14.3 화면에서 필요한 상태

생산대시보드의 가공 provision 영역은 최소 아래 상태를 표시해야 한다.

| 상태 | 의미 |
| --- | --- |
| `mes_reported` | MES 보고가 있는 정상 row |
| `manual_open` | 수기 보정만 있고 아직 MES 매칭 전 |
| `manual_matched` | 수기 보정이 MES와 매칭 완료 |
| `manual_partial` | 수기 보정 일부만 MES와 매칭 |
| `manual_mismatch` | 수기와 MES 수량 또는 키가 맞지 않음 |
| `needs_review` | 계획은 있으나 MES/수기 모두 없음 |
| `unplanned_mes` | 계획 없이 MES 보고만 있음 |

## 14.4 생산 담당자의 실제 사용 방식

생산 담당자는 모든 row를 다시 입력하지 않는다.

해야 하는 일:

- `생산했으나 MES 없음` 필터를 본다.
- 선진행 또는 작업지시 미생성으로 누락된 row만 선택한다.
- 양품 수량, 불량 유형/수량, 사유를 입력한다.
- 미대사 또는 불일치 큐를 확인한다.

하지 않아야 하는 일:

- MES에 이미 있는 정상 row를 다시 수기로 입력하지 않는다.
- 계획 없는 품목을 임의로 신규 생성하지 않는다.
- 수기 보정을 최종 원장처럼 쓰지 않는다.

이렇게 해야 현장 부담을 줄이면서도 진행 공유와 중복 방지를 동시에 달성할 수 있다.

초기 UI 배치는 별도 가공 화면보다 `/production` 대시보드 안의 가공 진행 카드와 상세 모달을 우선한다. 담당자는 대시보드에서 MES 진행률을 보다가 누락 row만 바로 보정해야 하며, 별도 화면 이동은 대사 큐가 커진 뒤에 분리한다.
