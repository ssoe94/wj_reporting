...
export default function DateRecordsTable({ date }: Props) {
  ...
  const handleSave = async (data: Partial<Report>) => {
    ...
    try {
      ...
      toast.success(t('save_success') || '수정되었습니다');
      queryClient.invalidateQueries({ queryKey: ['reports'] });
      setEditing(false);
      setDetail(null);
    } catch {
      toast.error(t('save_fail') || '수정 실패');
    }
  };
      ...
      <table className="min-w-full text-sm border border-gray-400 rounded-md border-separate border-spacing-0 mt-4">
        ...
        <tbody>
          {list.map((r) => (
            <tr
              key={r.id}
              className="border-t border-gray-200 last:border-b-0 hover:bg-blue-50 cursor-pointer"
            ...
            >
              ...
              <td className="px-2 py-1 text-center">
                {!r.actual_qty ? '-' : ((r.operation_time * 60) / r.actual_qty).toFixed(1)}
              </td>
              <td className="px-2 py-1 text-center">
                {(() => {
            ...
            {!editing ? (
              <>
                ...
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <span className="text-gray-500">{t('start_dt')}</span>
                  <span>
                    {detail.start_datetime
                      ? dayjs(detail.start_datetime).format('YYYY-MM-DD HH:mm')
                      : '-'}
                  </span>
                  <span className="text-gray-500">{t('end_dt')}</span>
                  <span>
                    {detail.end_datetime
                      ? dayjs(detail.end_datetime).format('YYYY-MM-DD HH:mm')
                      : '-'}
                  </span>
                  <span className="text-gray-500">{t('total_time')}</span>
                  <span>
                    {detail.total_time
                      ? `${detail.total_time}${t('minutes_unit')}`
                      : '-'}
                  </span>
                  <span className="text-gray-500">{t('idle_time')}</span>
                  <span>
                    {detail.total_time && detail.operation_time
                      ? `${detail.total_time - detail.operation_time}${t('minutes_unit')}`
                      : '-'}
                  </span>
                  <span className="text-gray-500">{t('run_time')}</span>
                  <span>
                    {detail.operation_time
                      ? `${detail.operation_time}${t('minutes_unit')}`
                      : '-'}
                  </span>
                  <span className="text-gray-500">{t('header_note')}</span>
                  <span className="col-span-1">{detail.note}</span>
                </div>
              ...
              <>
                <Dialog.Title className="text-lg font-bold">
                  {dayjs(detail.date).format('YYYY.MM.DD')} – {detail.machine_no}号机 {t('edit')}
                </Dialog.Title>
                <ReportForm
                  initialData={detail}
                  onSave={handleSave}
