...
const copy = {
  ko: { title: "품번 C/T 요약", close: "닫기", live: "현재 현황판", machine: "호기", recent: "설비 C/T · 최근 60분", stale: "수집 지연", history: "최근 30일 품번 C/T", average: "기간 추정 C/T", date: "생산일", ct: "추정 C/T", machines: "설비", loading: "보존된 요약을 불러오는 중입니다.", error: "이력 요약을 불러오지 못했습니다.", retry: "다시 시도", empty: "이 품번에 연결된 보존 기록이 없습니다.", basis: "생산일: 상하이 08:00~다음 날 08:00. 종료된 시간대의 보존 기록이며, 근거 없는 값은 —로 표시합니다.", estimate: "품번 연결에 생산계획 순서에 따른 추정이 포함될 수 있습니다.", liveHint: "현재 값은 설비 기준입니다. 복수 품번을 함께 생산하는 경우 품번별 실측 C/T가 아닙니다." },
  zh: { title: "料号 C/T 概览", close: "关闭", live: "当前看板", machine: "号机", recent: "设备 C/T · 最近60分钟", stale: "采集延迟", history: "最近30天料号 C/T", average: "期间估算 C/T", date: "生产日", ct: "估算 C/T", machines: "设备", loading: "正在读取保存的概览。", error: "无法读取历史概览。", retry: "重试", empty: "暂无与此料号关联的保存记录。", basis: "生产日：上海08:00~次日08:00。显示已结束时段的保存记录，缺少依据的值显示为 —。", estimate: "料号关联可能包含按生产计划顺序的估算。", liveHint: "当前值按设备统计。多料号同时生产时，不代表各料号的单独实测 C/T。" },
};
...
export function BoardPartSummaryModal({ partNo, businessDate, machines, language, onClose }: {
  ...
  const query = useQuery({
    ...
    queryFn: async ({ signal }) => (await http.get<{ start_date: string; end_date: string; cycle_time_seconds: number | null; daily: { business_date: string; cycle_time_seconds: number | null; machine_numbers: number[] }[] }>(`/injection/board-part-cycle-time/?${new URLSearchParams({ part_no: partNo })}`, { signal })).data,
    ...
    <div ref={ref} className="board-part-summary" role="dialog" aria-modal="true" aria-labelledby={titleId} tabIndex={-1}>
      ...
      <section aria-label={text.history}>
        ...
        {query.isPending ? <p role="status">{text.loading}</p> : query.isError ? <div role="alert"><p>{text.error}</p><button type="button" onClick={() => { void query.refetch(); }}>{text.retry}</button></div> : query.data?.daily.some((day) => day.machine_numbers.length) ? <>
          <div className="board-part-summary__average"><span>{text.average}</span><strong>{ct(query.data.cycle_time_seconds)}</strong></div>
