...
// A cold Render instance can take longer than the shared 30-second API
// timeout while it loads the durable MES datasets and builds the first
// overview. Keep the longer allowance scoped to this read-heavy endpoint.
const RAW_MATERIAL_OVERVIEW_TIMEOUT_MS = 90_000;
...
export async function getRawMaterialOverview(params: RawMaterialOverviewParams): Promise<RawMaterialOverview> {
  const response = await http.get("/inventory/raw-materials/overview/", {
    ...
    timeout: RAW_MATERIAL_OVERVIEW_TIMEOUT_MS,
  });
