...
// The stored raw-material report API is intentionally bounded to the 30-day
// dataset collected by the daily sync. Do not offer periods that the backend
// rejects with HTTP 400.
const RAW_MATERIAL_LOOKBACK_OPTIONS = [7, 14, 30] as const;
...
export function RawMaterialManagementPage() {
  ...
  const overviewQuery = useQuery({
    queryKey,
    queryFn: () => getRawMaterialOverview(params),
    // Retry one transient cold-start/cache-miss failure, without repeatedly
    // starting the same expensive report build.
    retry: 1,
  });
