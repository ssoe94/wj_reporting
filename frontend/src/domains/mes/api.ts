...
async function fetchInjectionProductionMatrix(date?: string) {
  if (date) {
    try {
      ...
      return normalizeInjectionProductionMatrix(historicalResponse.data);
    } catch {
      const historicalFallbackParams = new URLSearchParams({
        interval: "30min",
        columns: "49",
        date,
      });
      ...
      return normalizeInjectionProductionMatrix(historicalFallbackResponse.data);
    }
  }
