...
# Validate the Gemma runtime and every indexed weight shard before replacing
# any running LaunchAgent or its plist.
"$repo_dir/scripts/start-local-gemma-llm.sh" --check-model
...
render_plist "$script_dir/$gemma_label.plist.template" "$gemma_target"
