...
The AI runtime is tiered (current contract: `docs/ai/2026-09-21-chatgpt-daily-analysis.md`). The routine tier stays on-device `qwen38` (`local_worker/`): hourly briefings, interactive questions, daily quality attention and classification audits use the outbound `ai_core` queue. The deep tier is `chatgpt`, run daily at 09:00 Asia/Shanghai by the ChatGPT desktop task through `local_worker/chatgpt_bridge.py`. The server builds seven-day production/quality evidence ending on the latest closed business date. The bridge verifies the deployed contract before any queue mutation; an app schedule, backend deployment, accepted result and observed dashboard are separate facts. Claude is retired from worker capabilities by the owner's 2026-09-21 decision; retain existing Claude result identity only as history, and do not resume its scheduler. Both tiers explain server-provided evidence and never calculate production metrics. Verify `accepted_explanation` after submit; HTTP success alone is not accepted analysis. UI shows model names from server data. Preserve persisted local identifiers (`qwen38`, `local_llm_rewrite`, `qwen_classification`, prompt versions and `claimed_by` names) to retain history.
...
- `/boards/overview` keeps its 3×3 layout and original light palette. Its logo is an image-only light/dark toggle; do not add a mode badge to the logo. Default display hours use Asia/Shanghai: light 08:00–20:00, dark otherwise. A manual choice survives reload only until the next 08:00/20:00 boundary, independently of production business dates and observed weather.
- Board dark colors are scoped `--board-*` tokens, including charts, weather, AI and empty/error states. Chinese card and central titles use weight 900. Preserve shared date/place, clock/temperature and description/observation alignment, and the centered translucent caption below the globe.
- The overview header has no bottom graphic/freshness row. Keep its generated-at time in the status strip and refresh-failure feedback in the existing refresh control. The user-approved crystal artwork retains reduced-motion support; other panel rotation controls remain available.
